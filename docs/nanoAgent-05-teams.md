...
> **关于本篇代码的说明：**
>
> 和第四篇一样，本篇的 `agent-teams.py` 是我们**新开发的文件**（[GitHub 源码](https://github.com/GitHubxsy/nanoAgent/blob/main/agent-teams.py)），不在 nanoAgent 原始仓库中。它在 `agent-subagent.py` 的基础上，用两个类（`Agent` + `Team`）实现了多智能体团队协作。
...
但能力越强，副作用也越大——Agent 干的活越多、协作越复杂，`messages` 就越长。长到撑爆 LLM 的 context window 怎么办？在 [第六篇：上下文压缩](./nanoAgent-06-compact.md) 中，我们用一个 30 行的函数来解决这个"自我窒息"问题。

---

*本文基于 agent-teams.py（[GitHub 源码](https://github.com/GitHubxsy/nanoAgent/blob/main/agent-teams.py)）分析。完整系列：[第一篇](./nanoAgent-01-essence.md) → [第二篇](./nanoAgent-02-memory.md) → [第三篇](./nanoAgent-03-skills-mcp.md) → [第四篇](./nanoAgent-04-subagent.md) → 第五篇（本文） → [第六篇](./nanoAgent-06-compact.md)*
